...
pub struct EXP3 {
    ...
    waiting_reward: bool,
    enable_history: bool,
    history: Vec<EXP3Round>,
}

/// An EXP3 instance
impl EXP3 {
    pub fn new(no_actions: usize, gamma: f64, enable_history: bool) -> EXP3 {
        EXP3 { 
            no_actions,
    ...
    pub fn probabilities(&self) -> Vec<f64> {
        ...
        self.weights.iter().map(|w| {
            ((1.0 - self.gamma) * (w / weights_sum)) + 
            (self.gamma / self.no_actions as f64)
        }).collect()
    }
    ...
    pub fn give_reward(&mut self, action: usize, reward: f64) {
        ...
        assert!(0.0 <= reward && reward <= 1.0, "Reward {reward} is not comprised in [0, 1]");
        assert!(self.waiting_reward, "EXP3 has not taken an action yet");
